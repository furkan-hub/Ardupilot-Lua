...
SHIP_WPOFFSET     = bind_add_param('WPOFFSET', 4, 0)
SHIP_WPOFFSET_TH     = bind_add_param('WPOFFSET_TH', 5, 1)
...
function update_wp(offset_treshold)                                             -- waypointleri öteler
   if follow:have_target() then

       home_loc = ahrs:get_home()                                              -- home noktasını al

       home_distance = target_pos:get_distance(home_loc)                       -- eve olan uzaklığı hesapla

       bearing_wp = math.deg(home_loc:get_bearing(target_pos))                 -- iki nokta arasında ki eğimi hesaplar
       --gcs:send_text(0, "beacon is ok")

       --gcs:send_text(0, "offset: " .. tostring(home_distance).."bearing: "..tostring(bearing_wp))

       if home_distance > offset_treshold then
           for i = 1, mission:num_commands() - 1 do
               ...
               if lat and lon ~= 0 then
                   ---gcs:send_text(0,"old lat: "..tostring(locs[1]).." old lon: "..tostring(locs[2]))
                   loc_var = Location()
                   loc_var:lat(locs[1])
                   loc_var:lng(locs[2])
                   loc_var:offset_bearing(bearing_wp, home_distance)
                   ---gcs:send_text(0,"new lat: "..tostring(loc_var:lat()).." new lon: "..tostring(loc_var:lng()))
                   home = ahrs:set_home(target_pos) -- home güncelle
                   item:x(loc_var:lat())
                   item:y(loc_var:lng())
...
function update()
   ...
   if SHIP_WPOFFSET:get() == 1 then-- benim ekledigim kısım
      treshold = SHIP_WPOFFSET_TH:get()
      update_wp(treshold)
   end
